...
import re
from hommi.demonstration_processing.utils.color_util import red, green, yellow
...
def get_gripper_calibration_run_dir(demonstration_dir, side):
    ...
    # print(f"Gripper calibration run directory: {gripper_cal_run_dir}")
...
def keep_demonstration(demonstration_title, demonstration_json_path, filters: DictConfig, demo_type=None, has_gopro=False):
    ...
    try:
        ...
        print(f"Error reading {demonstration_json_path}: {e}")
        return False
...
def iterate_demonstrations(demonstrations_dir, filters: DictConfig, demo_type=None, has_gopro=False):
    ...
    for demonstration_dir in sorted(list(glob(demonstrations_dir + '/*/*'))):
        print(f'Processing {demonstration_dir}...')
        ...
        if not (os.path.exists(os.path.join(demonstration_dir, f'left.json')) and os.path.exists(os.path.join(demonstration_dir, f'right.json')) and os.path.exists(os.path.join(demonstration_dir, f'head.json'))):
            print(f'{demonstration_dir} does not have all sides')

...
from hommi.demonstration_processing.utils.generic_util import get_gripper_calibration_run_dir, get_demonstration_video_frame_count, is_iphone_only_demonstration, get_demonstration_json_data
...
from hommi.common.contants import IPHUMI_ULTRAWIDE_OFFSET_FROM_CENTER_X, IPHUMI_ULTRAWIDE_OFFSET_FROM_FINGER_AR_TAG_Z, IPHUMI_TCP_OFFSET_FROM_MAIN_CAMERA

def get_demo_gripper_width(demonstration_dir, side: str, include_detection_types: bool=False, aligned_video_times=None):
    if side == 'head':
        raise ValueError('Head side does not have gripper width data')
    if is_iphone_only_demonstration(demonstration_dir):
        return get_demo_gripper_width_iphone(demonstration_dir, side, include_detection_types, aligned_video_times)
    else:
        assert not include_detection_types
        return get_demo_gripper_width_gopro(demonstration_dir, side)


def get_demo_gripper_width_iphone(demonstration_dir, side: str, include_detection_types=False, aligned_video_times=None):
...
def iphone_to_tcp_poses(iphone_poses, not_use_iphone_tcp_offset=False, no_mount_rotation=True, rotate_z_pi=False):
    ...
    if rotate_z_pi:
        # the iphone is mounted rotated around z axis for 180, with the charging port pointing to the left
        z_rotation_180 = np.array([[-1, 0, 0],
                                   [0, -1, 0],
                                   [0, 0, 1]])
        arkit_tcp_rot = z_rotation_180 @ np.array([[1, 0, 0],
                                                   [0, -1, 0],
                                                   [0, 0, -1]])
    arkit_tcp_rot = np.array([[1,0,0],
                            [0,-1,0],
                            [0,0,-1]])
    if no_mount_rotation:
        iphone_tcp_rot = arkit_tcp_rot
    else:
    ...
    if not not_use_iphone_tcp_offset:

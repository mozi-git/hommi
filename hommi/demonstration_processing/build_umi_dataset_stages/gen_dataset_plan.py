...
from hommi.demonstration_processing.utils.generic_util import get_demonstration_type, get_demonstration_sides_present, get_demonstration_json_data, read_demonstration_metadata
...
def gen_dataset_plan(session_dir: str, out_plan_path: str, task_filters=None, rotate_head=False):
    # all_plans = [{
    #     "grippers": [{
    #         "tcp_pose": np.ndarray,
    ...
    for demo_name in os.listdir(demos_dir):
        ...
        aligned_times = read_demonstration_metadata(demo_dir)
        for side in sides_present:
            ...
            arkit_pose_path = os.path.join(demo_dir, f'camera_trajectory_iphone_{side}.csv')
            df = pd.read_csv(arkit_pose_path)
            arkit_poses = df[['x', 'y', 'z', 'q_x', 'q_y', 'q_z', 'q_w']].values
            arkit_poses = pos_quat_xyzw_to_4x4(arkit_poses)
            T = arkit_poses.shape[0]
            
            """ gripper data """
            print(f'Processing {side} side of {demo_name}')
            tcp_pose = mat_to_pose(iphone_to_tcp_poses(arkit_poses, not_use_iphone_tcp_offset=(side == 'head'), rotate_z_pi=(rotate_head and side == 'head')))
            try:
                gripper_width = get_demo_gripper_width(demo_dir, side, include_detection_types=False, aligned_video_times=aligned_times[f'{side}_frame_times'])
            ...
            """ camera data """
            if os.path.exists(os.path.join(demo_name, f'{side}_rgb_masked.mp4')):
            ...
            # find pose idx to video frame idx mapping, aligned_times -> json_data['rgbTimes'], json_data['depthTimes'], json_data['ultrawideRGBTimes']
            # to do this, for each pose, we find the closest video frame time that is before or equal to the pose time
            pose_idx_to_main_idx = np.zeros(T, dtype=int)
            pose_idx_to_depth_idx = np.zeros(T, dtype=int)
            pose_idx_to_ultrawide_idx = np.zeros(T, dtype=int)
            aligned_timestamps = np.array([datetime_fromisoformat(x).timestamp() if x != "" else 0 for x in aligned_times[f'{side}_frame_times']])
            main_timestamps = np.array([datetime_fromisoformat(x).timestamp() if x != "" else 0 for x in json_data['rgbTimes']])
            depth_timestamps = np.array([datetime_fromisoformat(x).timestamp() if x != "" else 0 for x in json_data['depthTimes']])
            ultrawide_timestamps = np.array([datetime_fromisoformat(x).timestamp() if x != "" else 0 for x in json_data['ultrawideRGBTimes']])
            pose_idx_to_main_idx[:] = np.searchsorted(main_timestamps, aligned_timestamps, side='right') - 1
            pose_idx_to_depth_idx[:] = np.searchsorted(depth_timestamps, aligned_timestamps, side='right') - 1
            pose_idx_to_ultrawide_idx[:] = np.searchsorted(ultrawide_timestamps, aligned_timestamps, side='right') - 1
            plan[f'cameras_{side}'] = [{
                'main_video_path': main_video_path,
                'depth_video_path': depth_video_path,
                'ultrawide_video_path': ultrawide_video_path,
                'pose_idx_to_main_idx': pose_idx_to_main_idx,
                'pose_idx_to_depth_idx': pose_idx_to_depth_idx,
                'pose_idx_to_ultrawide_idx': pose_idx_to_ultrawide_idx
                # 'main_idx_to_ultrawide_idx': pose_idx_to_ultrawide_idx,
                # 'main_video_start_end': (0, T),
                # 'ultrawide_video_start_end': (0, len(ultrawide_timestamps))
            }]

...
import cv2
from hommi.common.trajectory_util import sample_poses_at_times, save_trajectory_umi_format
from hommi.common.transform_util import pose_4x4_to_quat_xyzw, pos_quat_xyzw_to_4x4
import numpy as np
import math
from hommi.demonstration_processing.utils.generic_util import get_demonstration_path, write_demonstration_metadata
from hommi.common.timecode_util import datetime_fromisoformat
from hommi.demonstration_processing.utils.color_util import red
from hommi.demonstration_processing.utils.gopro_util import get_gopro_start_video_time
from hommi.demonstration_processing.utils.generic_util import get_demonstration_sides_present
...
def align_multi_iphone_data(demonstration_iterator, cfg: DictConfig):
    ...
    for demonstration_dir in demonstration_iterator('demonstration'):
        ...
        # Skip if already aligned and not overwriting
        if all(os.path.exists(os.path.join(demonstration_dir, f'camera_trajectory_iphone_{side}.csv')) for side in sides) and not cfg.overwrite:
            num_already_processed += 1
            continue
        ...
        def load_pose_times(pose_data):
            times = [datetime_fromisoformat(x).timestamp() for x in pose_data['poseTimes']]
            assert len(times) == len(pose_data['poseTransforms'])
            return times

        pose_data = {side: load_pose_data(os.path.join(demonstration_dir, f'{side}.json')) for side in sides}
        pose_times = {side: load_pose_times(pose_data[side]) for side in pose_data}
        ...
        # Get FPS from video
        fps = cfg.fps if hasattr(cfg, 'fps') else 60
        dt = 1 / fps
        sample_times = np.arange(valid_start_time, valid_end_time, dt)

        sampled = {}
        for side in sides:
            poses = np.array(pose_data[side]['poseTransforms'])
            times = pose_times[side]
            pose_samples = sample_poses_at_times(poses, times, sample_times)
            sampled[side] = pose_samples

        # Use the same sample_times for all sides
        for side in sides:
            poses = sampled[side]
            is_lost = np.zeros(len(sample_times), dtype=bool)
            try:
                save_trajectory_umi_format(
                    os.path.join(demonstration_dir, f'camera_trajectory_iphone_{side}.csv'),
                    poses, sample_times, is_lost
                )
                time_strs = [datetime.fromtimestamp(t, timezone.utc).isoformat() for t in sample_times]
            except Exception as e:
                print(f"Error saving trajectory for {side}: {e}, {demonstration_dir}")
                exit(1)
            write_demonstration_metadata(demonstration_dir, {f'{side}_frame_times': time_strs})

...
from hommi.demonstration_processing.utils.generic_util import write_demonstration_metadata, keep_demonstration, get_demonstration_path, demonstration_to_display_string
...
def group_iphone_data(cfg: DictConfig):
    ...
    for demonstration_json_path in demonstration_files:
        ...
        for in_file in demonstration_in_files:
            ...
            # Apply side swapping if enabled (not for gripper calibration files)
            if hasattr(cfg, 'swap_sides') and cfg.swap_sides and 'grippercalibration' not in demonstration_title:
                if out_name.startswith('left_'):
                    out_name = out_name.replace('left_', 'right_', 1)
                elif out_name.startswith('right_'):
                    out_name = out_name.replace('right_', 'left_', 1)

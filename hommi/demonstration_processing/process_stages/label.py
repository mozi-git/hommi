...
from hommi.demonstration_processing.utils.generic_util import demonstration_to_display_string, get_demonstration_sides_present, get_demonstration_video_fps, read_demonstration_metadata, get_demonstration_video_frame_count, get_demonstration_property, get_demonstration_main_video_path, get_demonstration_frame_times
from hommi.common.timecode_util import datetime_fromisoformat
from hommi.demonstration_processing.utils.gripper_util import get_demo_gripper_width
...
def identify_tasks(demonstration_dir, cfg: DictConfig) -> List[Task]:
    """Returns a list of task segments across all sides present in the demonstration."""
    sides_present = get_demonstration_sides_present(demonstration_dir)
    ...
    for side in sides_present:
        ...
        if label_type == 'Narration':
            ...
            if side == 'head':
                print(f"[{side}] Ignoring GripperWidth labelType for head; head should not use gripper width.")
                tasks = []
            else:
                tasks = identify_tasks_gripper_width(demonstration_dir, cfg, side)
        elif label_type == 'Predefined':
            tasks = identify_tasks_labeled(demonstration_dir, cfg, side)
        elif label_type == 'None':
            tasks = []
        ...
        if cfg.visualize and len(tasks) > 0:
            visualize_tasks(demonstration_dir, tasks, side)
...
def identify_tasks_labeled(demonstration_dir, cfg: DictConfig, side: str) -> List[Task]:
    ...
    for i in range(len(task_names)):
        ...
        end_frame_idx = min(int(rel_task_end_timestamp * fps), episode_frame_count)
...
def identify_tasks_gripper_width(demonstration_dir, cfg: DictConfig, side: str) -> List[Task]:
...
def visualize_tasks(demonstration_dir: str, tasks: List[Task], side: str):
    ...
    output_path = os.path.join(demonstration_dir, f'subtasks_{side}.mp4')

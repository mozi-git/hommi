import torch
import einops
import hommi.train_network.model.vision_3d.pytorch3d_transforms as p3d
...
def show_point_cloud_viser(
    ...
    if line_segments is not None:
        if isinstance(line_segments, torch.Tensor):
            line_segments = line_segments.detach().cpu().numpy()
        if isinstance(line_colors, torch.Tensor):
            line_colors = line_colors.detach().cpu().numpy()
        if line_colors is None:
            line_colors = np.ones((line_segments.shape[0], 3), dtype=np.float32)
        try:
            ...
            print(f"Failed to add line segments to viser: {exc}")
...
def rot6d_to_mat(d6):
    ...
    b2 = normalize(a2 - dot * b1)
    b3 = torch.cross(b1, b2, dim=-1)
    return torch.stack((b1, b2, b3), dim=-2)  # shape (..., 3, 3)

def pose_to_mat(d10):
    pos = d10[..., :3]
    d6 = d10[..., 3:]
    rotmat = rot6d_to_mat(d6)
    batch_shape = d10.shape[:-1]
    out = torch.zeros(batch_shape + (4, 4), dtype=d10.dtype, device=d10.device)
    out[..., :3, :3] = rotmat
    out[..., :3, 3] = pos
    out[..., 3, 3] = 1
    return out

...
class TransformerObsEncoder(ModuleAttrMixin):
    def __init__(self,
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == 'rgb':
                ...
                low_dim_total_dims += dim
                if not concat_low_dims:
                    proj = nn.Identity()
                    if dim != n_emb:
        ...
        if concat_low_dims:
            proj = nn.Identity()
            if low_dim_total_dims != n_emb:
                proj = nn.Linear(in_features=low_dim_total_dims, out_features=n_emb)
            key_projection_map['low_dim'] = proj
        ...
        self.concat_low_dims = concat_low_dims
    ...
    def forward(self, obs_dict):
        ...
        if self.concat_low_dims:
            ...
            low_dim_data = torch.cat(low_dim_data, -1)
            low_dim_emb = self.key_projection_map['low_dim'](low_dim_data)
            assert low_dim_emb.shape[-1] == self.n_emb
            embeddings.append(low_dim_emb)
        else:
            for key in self.low_dim_keys:
                data = obs_dict[key]
                B, T = data.shape[:2]
